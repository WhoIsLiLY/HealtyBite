...
@section('content')
    <div class="min-h-screen bg-gray-100 px-4 pt-4 pb-20">
        ...
        <div class="w-full lg:mb-0 align-left p-4 flex">
            <h1 class="text-2xl font-bold text-green-600">Edit Menu</h1>
        </div>
        <div class="flex items-start bg-white rounded-xl shadow-sm hover:shadow-md transition p-4">
            <form action="{{ route('restaurant.menus.update', $menus->id) }}" method="POST" enctype="multipart/form-data" class="w-full space-y-4">
                @method('PUT')
                @csrf
                    ...
                    <div class="w-full md:w-1/2 space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">
                                ...
                                @if ($menus->menu_image)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/' . $menus->menu_image) }}" alt="Gambar Menu"
                        ...
                        <div>
                            ...
                            <div class="mt-2 flex items-center space-x-6">
                                <label class="inline-flex items-center">
                                    <input type="radio" name="tersedia" value="1" class="text-green-600 focus:ring-green-500"
                                    {{ old('tersedia', $menus->isAvailable) == 1 ? 'checked' : '' }} required>
                                    <span class="ml-2 text-gray-700">Iya</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input type="radio" name="tersedia" value="0" class="text-green-600 focus:ring-green-500" 
                                    {{ old('tersedia', $menus->isAvailable) == 0 ? 'checked' : '' }} required>
                        ...
                        <div>
                            ...
                            <div id="tagsContainer" class="space-y-4 mt-2">
                                @foreach ($menus->foodTags as $tag)
                                <div class="flex flex-col md:flex-row md:space-x-2 space-y-2 md:space-y-0 items-start">
                                    <!-- Nama tag -->
                                    <div class="flex flex-col flex-1">
                                        <label class="text-sm font-medium text-gray-700 mb-1">
                                            Nama Tags <span class="text-red-500">*</span>
                                        </label>
                                        <input type="text" name="tags_name[]" value="{{ $tag->name }}" required
                                    ...
                                    <div class="flex flex-col flex-1">
                                        <label class="text-sm font-medium text-gray-700 mb-1">
                                            Deskripsi Tags <span class="text-red-500">*</span>
                                        </label>
                                        <input type="text" name="tags_description[]" value="{{ $tag->description }}" required
                                            class="rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
                                    </div>

                                    <!-- Id tag -->
                                    <input type="hidden" name="tags_id[]" value="{{ $tag->id }}">

                                    <!-- Tombol hapus -->
                                    <button type="button" class="text-red-600 hover:text-red-800 mt-6 md:mt-8" onclick="this.closest('div.flex').remove()">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5-4h4m-4 0
                    ...
                    <div class="md:w-1/2 space-y-4">
                        ...
                        <div>
                            ...
                            <div id="addonsContainer" class="space-y-4 mt-2">
                                @foreach ($menus->addons as $index => $addon)
                                    <div class="flex flex-col md:flex-row md:space-x-2 space-y-2 md:space-y-0 items-start">
                                        <!-- Nama Addon -->
                                        <div class="flex flex-col flex-1">
                                            <label class="text-sm font-medium text-gray-700 mb-1">
                                                Nama Addon <span class="text-red-500">*</span>
                                            </label>
                                            <input type="text" name="addons_name[]" value="{{ $addon->name }}" required
                                                class="rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
                                        </div>
                                        ...
                                        <div class="flex flex-col flex-1">
                                            ...
                                            <input type="number" name="addons_price[]" value="{{ $addon->price }}" min="0" required
                                                class="rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
                                        </div>
                                        ...
                                        <div class="flex flex-col flex-1">
                                            ...
                                            <select name="addons_type[]" required
                                                class="rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
                                                <option value="extra" {{ $addon->type == 'extra' ? 'selected' : '' }}>Extra</option>
                                                <option value="topping" {{ $addon->type == 'topping' ? 'selected' : '' }}>Topping</option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="flex flex-col flex-1">
                                            ...
                                            <div class="flex space-x-4">
                                                <label class="inline-flex items-center">
                                                    <input type="radio" name="addons_available[{{ $index }}]" value="1"
                                                        {{ $addon->isAvailable ? 'checked' : '' }}
                                                        class="text-green-600 focus:ring-green-500">
                                                    <span class="ml-2 text-gray-700">Iya</span>
                                                </label>
                                                <label class="inline-flex items-center">
                                                    <input type="radio" name="addons_available[{{ $index }}]" value="0"
                                                        {{ !$addon->isAvailable ? 'checked' : '' }}
                                                        class="text-green-600 focus:ring-green-500">
                                                    <span class="ml-2 text-gray-700">Tidak</span>
                                                </label>
                                        ...
                                        <!-- Id addons -->
                                        <input type="hidden" name="addons_id[]" value="{{ $addon->id }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let addonIndex = {{ count($menus->addons) }};
